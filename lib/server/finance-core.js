...
function buildSubscriptionSnapshot({ subscriptions = [], period = 'today', now = new Date(), planPrices = {} }) {
  ...
  const subscriptionRows = [...subscriptions]
    .map((row) => {
      const startedAt = resolveSubscriptionDate(row);
      const expiresAt = row?.expires_at || null;
      const amount = resolveSubscriptionPrice(row, planPrices);
          ...
          status === 'trial' ? 'Trial' :
            status === 'suspended' ? 'Suspensa' :
              status === 'disabled' ? 'Desativada' :
                status === 'canceled' || status === 'cancelled' ? 'Cancelada' :
                  String(row?.status || 'Desconhecido');
