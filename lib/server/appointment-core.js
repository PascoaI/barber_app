...
function isCheckInWindowOpen(startDatetime, now = new Date(), minMinutes = 15, maxMinutes = 30) {
...
function validateAppointmentCreation(input) {
  ...
  // Regras de bloqueio de cliente/subscricao removidas em ambiente de testes.
  ...
  const hasOverlap = existingAppointments.some((a) => {
    if (!['awaiting_payment', 'pending', 'confirmed', 'in_progress'].includes(a.status)) return false;
