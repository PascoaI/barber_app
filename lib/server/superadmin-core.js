...
const ALLOWED_STATUS = new Set(['active', 'trial', 'suspended', 'disabled']);
const ALLOWED_PLAN = new Set(['free', 'basic', 'pro', 'enterprise']);

function normalizeStatus(value) {
  return ALLOWED_STATUS.has(value) ? value : 'active';
}

function normalizePlan(value) {
  return ALLOWED_PLAN.has(value) ? value : 'basic';
}
...
function canAdminLoginForBarbershop(user, barbershops) {
  ...
  if (!shop) return true;
  return !['disabled', 'suspended'].includes(shop.status);
}
